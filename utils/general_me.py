...
def coco80_to_coco91_class():  # converts 80-index (val2014) to 91-index (paper)
    ...
    cls90 = list(range(1,91))
    not_in_cls90 = [i for i in cls90 if i not in x]
    print(colorstr(f'coco80_to_coco91_class， not_in_cls90 (共{len(not_in_cls90)}个):\n'),not_in_cls90)
    return x

def xyxy2xywh(x:Union[torch.Tensor,np.ndarray]):
    # Convert nx4 boxes from [x1, y1, x2, y2] to [x, y, w, h] where xy1=top-left, xy2=bottom-right
    xshape = x.shape
    assert len(xshape)<=2 and xshape[-1]==4
    if len(xshape) == 1:
        x = torch.unsqueeze(x, dim=0) if isinstance(x, torch.Tensor) else np.reshape(x, (1, -1))
    ...
    y[:, 3] = x[:, 3] - x[:, 1]  # height
    if len(xshape)==1:
        x=torch.squeeze(x,dim=0) if isinstance(x, torch.Tensor) else np.squeeze(x,axis=0)
    return y
...
def strip_optimizer(f='best.pt', s=''):  # from utils.general import *; strip_optimizer()
    ...
    mb_o = os.path.getsize(f) / 1E6  # filesize
    ...
    print(f"Optimizer stripped from {f},{mb_o:.1f}MB;{(' saved as %s,' % s) if s else ''} {mb:.1f}MB")
